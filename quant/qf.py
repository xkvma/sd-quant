...
@torch.no_grad()
def quantize(w: torch.Tensor, axis=None, dtype: str = "uint8", qtype: str = "log"):
    if dtype == "uint8":
        qmin, qmax = 0.0, 255.0
    elif dtype == "uint4":
        qmin, qmax = 0.0, 15.0
    ...
    if qtype == "log":
        ...
        raise ValueError("qtype must be 'linear' or 'log'")
...
@torch.no_grad()
def dequantize(q: torch.Tensor, scale: torch.Tensor, zp: torch.Tensor, axis=None, qtype: str = "log", sign: torch.Tensor = None):
    if axis is None:
    ...
    if qtype == "log":
        if sign is None:
            raise ValueError("sign is required for log dequantization")
